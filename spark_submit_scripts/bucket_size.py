import boto3
import datetime
import os



access_key_id = '******************'
secret_access_key = '**************************'
session = Session(
        aws_access_key_id=access_key_id,
        aws_secret_access_key=secret_access_key,
        region_name='eu-central-1',
    )
s3b = session.client('s3')
s3 = s3b.resource('s3')
copy_source = {
    'Bucket': 'nlplive.humanindex.data',
    'Key': 'ajay_test'
}
s3.meta.client.copy(copy_source, 'nlplive.humanityindex.data', 'ajay_test')
print 'copied'

os._exit(0)
s3 = boto3.client('s3')

with open('myfile.txt', 'rb') as data:
    # response=s3.upload_fileobj(data, 'nlp.hi.data', 'analysis/analysis.txt')
    response=s3.put_object(ACL='public-read',Bucket='nlp.hi.data',Key='analysis/analysis.txt', Body=data, ContentType='text/html')

target=open('')

print response
print 'uploaded'
os._exit(0)




bucket_location = boto3.client('s3').get_bucket_location(Bucket='nlp.hi.data')
print bucket_location


object_url = "https://s3-{0}.amazonaws.com/{1}/{2}".format(
    bucket_location['LocationConstraint'],
    'nlp.hi.data',
    'analysis.txt')

print object_url


now = datetime.datetime.now()

cw = boto3.client('cloudwatch')
s3client = boto3.client('s3')

# Get a list of all buckets
allbuckets = s3client.list_buckets()

# Header Line for the output going to standard out
print('Bucket'.ljust(45) + 'Size in Bytes'.rjust(25))

# Iterate through each bucket
for bucket in allbuckets['Buckets']:
    # For each bucket item, look up the cooresponding metrics from CloudWatch
    response = cw.get_metric_statistics(Namespace='AWS/S3',
                                        MetricName='BucketSizeBytes',
                                        Dimensions=[
                                            {'Name': 'BucketName', 'Value': bucket['Name']},
                                            {'Name': 'StorageType', 'Value': 'StandardStorage'}
                                        ],
                                        Statistics=['Average'],
                                        Period=3600,
                                        StartTime=(now-datetime.timedelta(days=1)).isoformat(),
                                        EndTime=now.isoformat()
                                        )

    print response
    # The cloudwatch metrics will have the single datapoint, so we just report on it. 
    for item in response["Datapoints"]:
        print(bucket["Name"].ljust(45) + str("{:,}".format(int(item["Average"]))).rjust(25))
        # Note the use of "{:,}".format.   
        # This is a new shorthand method to format output.
        # I just discovered it recently. 